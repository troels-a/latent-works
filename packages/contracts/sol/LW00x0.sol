...
contract LW00x0 is ERC1155, ERC1155Supply, ERC1155Holder, Ownable, ReentrancyGuard, LTNTIssuer {
    ...
    function LTNTInfo(uint ltnt_id_, LTNT.Param memory param_) public view override returns(LTNT.Info memory){


        bytes memory image_ = _meta.getLTNTImage(ltnt_id_, param_);
    ...
    function onERC1155BatchReceived(address, address from_, uint[] calldata ids_, uint[] calldata, bytes calldata) public override returns(bytes4){
        ...
        if(ids_.length > 1 && ids_.length <= 7)
            _create(from_, ids_);

        for(uint i = 0; i < ids_.length; i++){
            _ltnt.issueTo(from_, LTNT.Param(ids_[i], from_, '', true), ids_.length > 1 && ids_.length <= 7 ? true : false);
        }
...
contract LW00x0_Meta {
    ...
    function getImage(uint comp_id_, bool mark_, bool encode_) public view returns(string memory) {
        ...
        LW00x0.CompInfo memory comp_ = LW00x0.CompInfo(
            Strings.toString(comp_id_),
            mark_,
            _00x0.getSeed(comp_id_, ''),
            '',
            '',
            '',
            _00x0.getSeed(comp_id_, 'rand'),
            _00x0.getWorks(comp_id_),
            '',
            '',
            '',
            0,
            0,
            _00x0.getOrientation(comp_id_),
            '',
            '',
            ['', ''],
            0,
        ...
        comp_.pos[0] = Strings.toString(Rando.number(comp_.seed, 100, comp_.orientation == LW00x0.Orientation.LANDSCAPE ? 500 : 800));
        comp_.pos[1] = Strings.toString(Rando.number(comp_.seed1, 100, comp_.orientation == LW00x0.Orientation.LANDSCAPE ? 800 : 500));

        comp_.width_string = comp_.orientation == LW00x0.Orientation.LANDSCAPE ? '700' : '1000';
        comp_.height_string = comp_.orientation == LW00x0.Orientation.LANDSCAPE ? '1000' : '700';
    ...
    function _getMark(uint comp_id_, LW00x0.CompInfo memory comp_) private pure returns(string memory){
        bytes memory comp_id_string_ = bytes(Strings.toString(comp_id_));

        bytes memory leading_zeroes_;
        if(comp_id_string_.length == 1)
            leading_zeroes_ = '00';
        if(comp_id_string_.length == 2)
            leading_zeroes_ = '0';

        string memory lift_text_ = Strings.toString((comp_.orientation == LW00x0.Orientation.LANDSCAPE ? 1000 : 700)-12);
